...
An expired or otherwise invalid registry entry does not quietly disable that one route. It makes the whole registry file unusable, so every route declared in it disappears until the registry is refreshed with `--apply` or the affected route is removed. This is a deliberate fail-closed choice: a stale route is treated as untrusted input rather than left running on unverified metadata.

The refusal lands where a route is used, not where a session starts. A session that never names an OpenRouter route still starts, reports the reason once, and runs with no OpenRouter workers. Naming a route from an unusable registry stops with that same reason. Stopping unrelated sessions bought no extra safety, because no unverified route was reachable from them in the first place, and it cost a great deal: an OpenAI-only or Grok-only session could not start, and the commands that repair the registry could not run either.
