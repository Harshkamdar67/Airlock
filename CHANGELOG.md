...
- The Fable route now serves Claude Fable 5.1 (`claude-fable-5-1[1m]`) instead of Claude Fable 5. The route name, cost category, and place in the handoff order are unchanged, so a saved `fable` choice simply starts the newer model. A `failover.json` written before this change names the old exact ID; that name stays accepted so the file does not fail the next launch, but its Fable entries are inert because no session routes the old model any longer. Run `airlock handoff recommended` again, or `airlock handoff set`, to rewrite the chains against the new default.
...
- Windows router daemons now bypass a virtual-environment Python launcher and use its base interpreter directly. Some launchers create a grandchild, so the process ID returned by `Popen` did not match the serving process ID in the protected ready marker; startup rejected the healthy router and supervision watched the wrong process.
- The configured Agent ceiling and the user-facing concurrency ceiling are separate again: snapshots may contain up to 64 named Agent definitions, while `airlock mode max-agents` accepts only the documented `1..20` concurrent workers.
