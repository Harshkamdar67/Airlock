...
- Effort controls now work for every root, including an OpenRouter one. Claude Code decides whether to show effort levels by matching the model ID against a list of known Claude patterns, so a GPT, Grok, or OpenRouter root previously stayed pinned to the default effort no matter what `/effort` selected. Profiles that seat non-Claude models now declare the effort capability tokens for every family slot they fill plus the custom model option, and set `CLAUDE_CODE_ALWAYS_ENABLE_EFFORT=1`, so `/effort` moves every route. Anthropic-rooted sessions stay untouched.
- The auto mode permission classifier now follows the cheapest model the session serves. Claude Code hard-codes its permission classifier to `claude-sonnet-5`, so on routes that do not serve that exact ID every tool call that is not obviously safe failed closed and asked for manual approval. A non-Anthropic root now seats the classifier on the session's small fast seat, which prefers Claude Haiku in a hybrid profile and rides Luna or Composer in pure profiles, instead of burning the premium root on every check. A single-model OpenRouter preset has nothing smaller than its own root, so there the root serves the classifier too. An Anthropic root clears the knob and leaves stock behaviour alone. Set `AIRLOCK_AUTO_MODE_MODEL` in the environment or the config file to override the choice with any exact model ID, or set it to `off` or `none` to clear the knob again. The same policy applies in both the bash launcher and the Python wrapper launcher.

### Fixed

- An OpenRouter-bound request carrying Anthropic server-side tools no longer dies with an opaque upstream error. Built-in WebSearch, WebFetch, code execution, and their history blocks are Anthropic API features, so forwarding them verbatim makes the upstream reject the whole request with a 400 before any model output. The session router now strips those declarations and the matching history blocks before forwarding, records what it removed as a diagnostic event, and replaces an emptied message with a short placeholder so the payload stays valid. Set `AIRLOCK_OPENROUTER_KEEP_SERVER_TOOLS=1` to restore the old pass-through behaviour. The router cannot fabricate tool results: server-side execution belongs to the client-to-Anthropic protocol, so a non-Anthropic root still has no working built-in WebSearch and should use the managed `airlock-web-tools` MCP server instead.
