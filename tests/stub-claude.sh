...
printf 'POLICY_HELPER=%s\n' "${AIRLOCK_POLICY_HELPER:-unset}"
printf 'SESSION_SNAPSHOT=%s\n' "${AIRLOCK_SESSION_SNAPSHOT:-unset}"
printf 'SNAPSHOT_SHA256=%s\n' "${AIRLOCK_SESSION_SNAPSHOT_SHA256:-unset}"
if [[ -n "${AIRLOCK_SESSION_SNAPSHOT:-}" && \
      -f "$AIRLOCK_SESSION_SNAPSHOT" && ! -L "$AIRLOCK_SESSION_SNAPSHOT" ]]; then
  printf 'SNAPSHOT_EXISTS=yes\n'
else
  printf 'SNAPSHOT_EXISTS=no\n'
fi
