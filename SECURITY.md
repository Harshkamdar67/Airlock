...
A route's verified metadata expires after 30 days. An expired or otherwise invalid registry entry makes the whole registry file unusable, so none of its declared routes can be selected until you refresh them with `airlock openrouter models refresh --apply` or remove the affected route with `airlock openrouter models remove ROUTE`.

The check applies where a route is used, not where a session starts. A session that never names an OpenRouter route still starts. It reports the reason once and runs with no OpenRouter workers. Naming a route from an unusable registry stops with that same reason, so nothing is ever routed on unverified metadata.
